...
object WizardDisplay {
  ...
  def getWizardGrid(db: Database): ScrollPane = {
    ...
    val weapons = variableTextArea(Dnd5Wizard.weaponProficiencies.mkString("\n"), 120, 100)

    val savingThrowsLabel = enhancedLabel("Saving Throw Proficiencies")
    val savingThrows = variableTextArea(Dnd5Wizard.savingThrowProficiencies.mkString("\n"), 150, 45)
    ...
    val skillChoices = variableTextArea(Dnd5Wizard.skillChoices.mkString("\n"), 150, 93)
    ...
    val chooseTraditionLevel = easyTextField(Dnd5Wizard.specializationStartLevel.toString)

    val buttonList = new ListBuffer[Button]
    val labelTraditions = enhancedLabel("Arcane Traditions")
    val traditionsList = getSpecializationNamesByClassName(db, Dnd5Wizard.classID.toString)
    val iterator = traditionsList.toIterator
    ...
    clericGrid.addRow(rowNum, labelTraditionLevel, chooseTraditionLevel, labelTraditions, buttonBox)
    rowNum += 1
  ...
  private case class WizardTableData(level: Int, cantrips: Int, slots1: Int, slots2: Int, slots3: Int,
  ...
  private def generateWizardTableData: ObservableBuffer[WizardTableData] = {
    ...
    for (i <- 1 to 20) {
      working += WizardTableData(i, Dnd5Wizard.cantripsPerLevel(i),
        Dnd5Wizard.firstLevelSlotsPerLevel(i), Dnd5Wizard.level2SlotsPerLevel(i),
        Dnd5Wizard.level3SlotsPerLevel(i), Dnd5Wizard.level4SlotsPerLevel(i), Dnd5Wizard.level5SlotsPerLevel(i),
        Dnd5Wizard.level6SlotsPerLevel(i), Dnd5Wizard.level7SlotsPerLevel(i), Dnd5Wizard.level8SlotsPerLevel(i),
  ...
  private def generateWizardTableView: TableView[WizardTableData] = {
    ...
    view.editable = false
    view.columns += sfx2jfx(new TableColumn[WizardTableData, String]("Wizard\nLevel") {
      editable = false
      sortable = false
      cellValueFactory = cdf => StringProperty(cdf.value.level.toString)
    })

    view.columns += sfx2jfx(new TableColumn[WizardTableData, String]("Cantrips\nKnown") {
      editable = false
      sortable = false
      cellValueFactory = cdf => StringProperty(cdf.value.cantrips.toString)
    })

    view.columns += sfx2jfx(new TableColumn[WizardTableData, String]("1st\nLevel\nSpell\nSlots") {
      editable = false
      sortable = false
      cellValueFactory = cdf => StringProperty(cdf.value.slots1.toString)
    })
    view.columns += sfx2jfx(new TableColumn[WizardTableData, String]("2nd\nLevel\nSpell\nSlots") {
      editable = false
      sortable = false
      cellValueFactory = cdf => StringProperty(cdf.value.slots2.toString)
    })
    view.columns += sfx2jfx(new TableColumn[WizardTableData, String]("3rd\nLevel\nSpell\nSlots") {
      editable = false
      sortable = false
      cellValueFactory = cdf => StringProperty(cdf.value.slots3.toString)
    })
    view.columns += sfx2jfx(new TableColumn[WizardTableData, String]("4th\nLevel\nSpell\nSlots") {
      editable = false
      sortable = false
      cellValueFactory = cdf => StringProperty(cdf.value.slots4.toString)
    })
    view.columns += sfx2jfx(new TableColumn[WizardTableData, String]("5th\nLevel\nSpell\nSlots") {
      editable = false
      sortable = false
      cellValueFactory = cdf => StringProperty(cdf.value.slots5.toString)
    })
    view.columns += sfx2jfx(new TableColumn[WizardTableData, String]("6th\nLevel\nSpell\nSlots") {
      editable = false
      sortable = false
      cellValueFactory = cdf => StringProperty(cdf.value.slots6.toString)
    })
    view.columns += sfx2jfx(new TableColumn[WizardTableData, String]("7th\nLevel\nSpell\nSlots") {
      editable = false
      sortable = false
      cellValueFactory = cdf => StringProperty(cdf.value.slots7.toString)
    })
    view.columns += sfx2jfx(new TableColumn[WizardTableData, String]("8th\nLevel\nSpell\nSlots") {
      editable = false
      sortable = false
      cellValueFactory = cdf => StringProperty(cdf.value.slots8.toString)
    })
    view.columns += sfx2jfx(new TableColumn[WizardTableData, String]("9th\nLevel\nSpell\nSlots") {
      editable = false
      sortable = false
